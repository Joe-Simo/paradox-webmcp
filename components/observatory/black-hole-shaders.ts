// Relativistic black-hole rendering adapted from the vgpu "black-hole"
// example (MIT, Vercel Labs). Paradox drives the disk with two semantic
// uniforms: divergence brightens the accretion flow as the recorded race
// approaches its commit, and violation shifts the thermal palette toward
// crimson when the invariant breaks.
...
fn volumeSample(point: vec3f, rayVelocity: vec3f) -> vec4f {
  ...
  thermal = mix(thermal, vec3f(1.0, 0.12, 0.2), params.violation * 0.55);
  ...
  let emission = thermal * density * doppler * gravitationalRedshift * (5.5 + 5.5 * params.divergence);
  return vec4f(emission, density * 2.1);
}

@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  let aspect = params.resolution.x / max(params.resolution.y, 1.0);
  let screen = (uv * 2.0 - 1.0) * vec2f(aspect, 1.0) - params.center;
...
export const brightPassWgsl = /* wgsl */ `
...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let contribution = max(soft * soft * knee, luminance - 1.0);
