// Effect and target orchestration for the baked black-hole pipeline,
// ported from the vgpu optimized example (MIT, Vercel Labs).

import { effect, sampler, target, type Effect, type Frame, type Gpu, type Surface, type Target } from "vgpu";

import { bakeWgsl, bloomWgsl, compositeWgsl, refineWgsl, shadeWgsl } from "./black-hole-shaders";
...
export interface Effects {
  ...
  bloomBlurV2: Effect;
  composite: Effect;
  postSampler: ReturnType<typeof sampler>;
  noiseVolume: NoiseVolume;
  noiseSampler: ReturnType<typeof sampler>;
}
...
const GBUFFER_FORMATS = ["rg32float", "rg32float", "rgba16float", "rgba16float"] as const;
const AA_FORMATS = ["rg8unorm", "rgba16float"] as const;
const CLEAR: readonly [number, number, number, number] = [0, 0, 0, 1];

export function createEffects(gpu: Gpu): Effects {
  const postSampler = sampler(gpu, { minFilter: "linear", magFilter: "linear" });
  return {
    bake: effect(gpu, bakeWgsl),
    refine: effect(gpu, refineWgsl),
    shade: effect(gpu, shadeWgsl),
    bloomExtract: effect(gpu, bloomWgsl),
    bloomBlurH0: effect(gpu, bloomWgsl),
    bloomBlurV0: effect(gpu, bloomWgsl),
    bloomDown1: effect(gpu, bloomWgsl),
    bloomBlurH1: effect(gpu, bloomWgsl),
    bloomBlurV1: effect(gpu, bloomWgsl),
    bloomDown2: effect(gpu, bloomWgsl),
    bloomBlurH2: effect(gpu, bloomWgsl),
    bloomBlurV2: effect(gpu, bloomWgsl),
...
export function createTargets(gpu: Gpu, size: readonly [number, number]): Targets {
  ...
  const postTarget = (targetSize: readonly [number, number]) =>
    target(gpu, { size: targetSize, colors: [{ format: "rgba16float" }] });
  ...
  try {
    return {
      gbuffer: own(target(gpu, { size: full, colors: GBUFFER_FORMATS.map((format) => ({ format })) })),
      aa: own(target(gpu, { size: full, colors: AA_FORMATS.map((format) => ({ format })) })),
...
export function setShadeUniforms(
  ...
  effects.shade.set({
    ...
    disk: { ...settings.disk, brightness: settings.disk.brightness * (0.72 + 0.5 * divergence) },
    stars: settings.stars,
  });
  effects.composite.set({
    composite: { params: [Math.max(0, settings.bloom.strength), violation, 0, 0] },
  });
}
...
export function renderChain(frame: Frame, effects: Effects, targets: Targets, output: Output, bake: boolean): void {
  if (bake) {
